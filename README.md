...
```
┌─────────────┐
│   SENSOR    │  Potenciômetro (GPIO 33 - ADC)
│  (Entrada)  │  └─> Lê: 0-4095 → BPM: 40-180
└──────┬──────┘
       │
       ▼
┌─────────────┐
...
| Tópico | Tipo | Descrição | Payload Exemplo |
|--------|------|-----------|-----------------|
| `medpulso/comando` | SUB | Recebe comandos de controle | `"LIGAR"`, `"DESLIGAR"` |
| `medpulso/status` | PUB | Envia status do sistema | `"Monitor ATIVADO"` |
| `medpulso/bpm` | PUB | Publica leituras de BPM | `"75 BPM - NORMAL"` |
...
- [Tutorial Arduino IDE + ESP32](https://randomnerdtutorials.com/installing-the-esp32-board-in-arduino-ide-windows-instructions/)
